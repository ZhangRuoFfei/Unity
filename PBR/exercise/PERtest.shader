...
Shader "PBR/PBRtest2" {
    Properties {
	_Color ("Main Color", Color) = (1,1,1,1)//基本颜色
	_SpecularColor ("Specular Color", Color) = (1,1,1,1)//镜面反射颜色
	_SpecularPower("Specular Power", Range(0,1)) = 1
	_SpecularRange("Specular Gloss",  Range(1,40)) = 0
	_Glossiness("Smoothness",Range(0,1)) = 1//表面平滑度
	_Metallic("Metallicness",Range(0,1)) = 0//金属度
	_Anisotropic("Anisotropic",  Range(-20,1)) = 0//各向异性
	_Ior("Ior",  Range(1,4)) = 1.5//折射因子，描述光穿过一个表面的速度
