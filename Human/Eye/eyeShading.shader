//copyright: shader from https://github.com/wolf96/shader_for_unity/tree/master/Realistic%20eye%20shading
//Some adjustments were made to achieve the target effect 
...
Shader "Custom/Eye Shading" {
	Properties{
		...
		//cubeMap的反射强度
		_ReflAmount("ReflAmount", Range(0, 2)) = 0.4
		//反射周围环境的cubeMap
	...
	SubShader
	{
		pass 
		{
			...
			float4 frag(v2f i) :COLOR
			{
				...
				float lerp1 = pow(8192, lerp(0, 0.5, _GLCornea - 0.4));
				float lerp2 = pow(8192, _GLCornea);
				float _SP = lerp(lerp1, lerp2, tex2D(_MaskTex, i.uv_MaskTex).a + 0.005);
				//_SP = pow(8192, lerp(0.5, 1.5, _GLIris + 0.1));
				float d = (_SP + 2) / (8 * PIE) * pow(dot(N, H), _SP);
				float f = _SCCornea + (1 - _SCCornea)*pow(2, -10 * dot(H, lightDir));
				float k = min(1, _GLIris + 0.545);
				float v = 1 / (k* dot(viewDir, H)*dot(viewDir, H) + (1 - k));
				float SpecIns = d*f*v;
				//if (tex2D(_MaskTex, i.uv_MaskTex).a < 0.02f)
				//SpecIns = 0.0f;
				//在这个地方控制了角膜的大小,可以调整uv_MaskTex改变
				//if (SpecIns < (-0.01f)) SpecIns = 0.0f;
				SpecIns = lerp(0, SpecIns, tex2D(_MaskTex, i.uv_MaskTex).a);
				...
				if (tex2D(_MaskTex, i.uv_MaskTex).a < 0.02)
					...
					roughSpecIns *= _SPIrisDetial.rgb;
				...
				diff = 0.3*diff1 + 0.7*diff2 + ambient;
				...
				float sub2 = 1;
				if (tex2D(_PupilMaskTex, UVs2).a < 0.02)
					sub2 = min(min(c.x, c.y), c.z)*tex2D(_PupilMaskTex, UVs2).a;
				else
					sub2 = max(max(c.x, c.y), c.z)*tex2D(_PupilMaskTex, UVs2).a;
